...
class TransNetV2(nn.Module):
    ...
    def __init__(self,
        ...
        output_dim = ((F * 2 ** (L - 1)) * 4) * 3 * \
            6  # 3x6 for spatial dimensions
...
class StackedDDCNNV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.pool = nn.MaxPool3d(kernel_size=(
            1, 2, 2)) if pool_type == "max" else nn.AvgPool3d(kernel_size=(1, 2, 2))
...
class FrameSimilarity(nn.Module):
    ...
    def forward(self, inputs):
        ...
        similarities = similarities_padded[batch_indices,
...
class ColorHistograms(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(
            lookup_window, output_dim) if output_dim is not None else None
        self.lookup_window = lookup_window
        assert lookup_window % 2 == 1, "`lookup_window` must be odd integer"
    ...
    def compute_color_histograms(frames):
        ...
        frames_flatten = frames.view(
            batch_size * time_window, height * width, 3)
        ...
        histograms.scatter_add_(0, binned_values, torch.ones(
